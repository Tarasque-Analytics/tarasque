...
/**
 * Parses a GICS code (8 digits) into its components.
 * Returns an object with sector, industryGroup, industry, and subindustry codes.
 */
export function parseGicsCode(gicsCode: string): {
  sector: string;
  industryGroup: string;
  industry: string;
  subindustry: string;
} {
  if (!gicsCode || gicsCode.length !== 8) {
    return { sector: "", industryGroup: "", industry: "", subindustry: "" };
  }
  return {
    sector: gicsCode.slice(0, 2),
    industryGroup: gicsCode.slice(2, 4),
    industry: gicsCode.slice(4, 6),
    subindustry: gicsCode.slice(6, 8),
  };
}

/**
 * Decodes a GICS code into readable sector/industry names.
 */
export function decodeGicsCode(gicsCode: string): {
  sector?: string;
  industryGroup?: string;
  industry?: string;
  subindustry?: string;
} {
  const { sector, industryGroup, industry, subindustry } = parseGicsCode(gicsCode);

  return {
    sector: sector ? GICS_SECTORS_REVERSE[sector] : undefined,
    industryGroup: industryGroup ? industryGroup : undefined,
    industry: industry ? industry : undefined,
    subindustry: subindustry ? subindustry : undefined,
  };
}

/**
 * Formats a GICS code as "XX-XX-XX-XX" for display.
 */
export function formatGicsCode(gicsCode: string): string {
  if (!gicsCode || gicsCode.length !== 8) return gicsCode;
  return `${gicsCode.slice(0, 2)}-${gicsCode.slice(2, 4)}-${gicsCode.slice(4, 6)}-${gicsCode.slice(6, 8)}`;
}

/**
 * Gets the sector name from a GICS code.
 */
export function getGicsSector(gicsCode: string): string | undefined {
  const sectorCode = gicsCode?.slice(0, 2);
  return sectorCode ? GICS_SECTORS_REVERSE[sectorCode] : undefined;
}

/**
 * Checks if a GICS code is valid (8 digits, non-zero).
 */
export function isValidGicsCode(gicsCode: string | null | undefined): boolean {
  if (!gicsCode) return false;
  return /^\d{8}$/.test(gicsCode) && gicsCode !== "00000000";
}
