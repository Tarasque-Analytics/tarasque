...
class YFinanceOptionsProvider:
    ...
    def _retry(self, fn, what: str, *, is_empty=None):
        ...
        if last_exc is not None:
            raise RuntimeError(
                f"yfinance {what} failed after {self._config.max_retries} attempts: {last_exc}"
            )
        return result
    ...
    def list_expiries(self, ticker: str) -> list[date]:
        ...
        raw = self._retry(
            ...
            is_empty=lambda r: (
                not r
            ),  # None / empty tuple → retry (yfinance occasionally returns ())
        )
        out: list[date] = []
        for s in raw or ():
            try:
        ...
        def _spot() -> Optional[float]:
            ...
            if fi is not None:
                for key in ("last_price", "lastPrice", "last_close", "previous_close"):
                    ...
                    ):  # 0.0 spot price is useless, but we can guard this explicitly
                        return f
                    ...
                    rows.append(
                        {
                            ...
                            "iv": iv,
                            "delta": bs_delta(
                                spot, strike, dte_days, iv, opt_type, r=risk_free_rate
                            ),
                        }
                    )
...
def _cap_strikes_per_side(rows: list[dict[str, Any]], spot: float, n: int) -> list[dict[str, Any]]:
    ...
    for group in groups.values():
        below = sorted((r for r in group if r["strike"] < spot), key=lambda r: spot - r["strike"])[
            :n
        ]
        atabove = sorted(
            (r for r in group if r["strike"] >= spot), key=lambda r: r["strike"] - spot
        )[:n]
